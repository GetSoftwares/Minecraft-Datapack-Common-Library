...
修改维度类型会导致加载世界时触发“实验性设置”警告，所以一般不修改维度类型。

原版的维度类型的 infiniburn 字段全部使用了标签，主世界为 #minecraft:infiniburn_overworld，下界为 #minecraft:infiniburn_nether，末地为 #minecraft:infiniburn_end。

那么只需要修改这三个标签就可以实现该玩法，而且不会触发“实验性设置”警告。

从原版数据包中提取这三个标签，然后将它们都修改为
...
然后将数据包拷进世界，退出重进（维度类型不会因为所引用的标签内容发生了变化而自动热重载），也可以实现方法一种的效果。
...
![默认情况下末影龙会摧毁绝大部分方块](image/默认情况下末影龙会摧毁绝大部分方块.gif)
